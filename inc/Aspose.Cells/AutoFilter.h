...
public:
    ...
    /// </summary>
    /// <returns>the area this filter applies to</returns>
    ASPOSE_CELLS_API CellArea GetCellArea();
    /// <summary>
    /// Gets the <see cref="CellArea"/> where the specified AutoFilter applies to.
    /// </summary>
    /// <param name="refreshAppliedRange">Whether refresh the applied range.
    /// For the applied range of auto filter, the last row may change when cells data changes.
    /// If this flag is true, then the last row of the range will be re-calculated according to current cells data.</param>
    /// <returns>the area this filter applies to</returns>
    ASPOSE_CELLS_API CellArea GetCellArea(bool refreshAppliedRange);
    /// <summary>
